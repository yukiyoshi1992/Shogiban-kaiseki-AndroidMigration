...
private fun GridConfirmStep(
    ...
    Column(
        ...
        Text(
            ...
            // ズレが原因で誤って不一致になったケースに対応できないという指摘を受け3択にしたが、
            // 「このまま進める」系のボタン（不一致の認識結果をそのまま確定する）は課題⑥で
            // 繰り返し誤操作（撮り直さずに確定してしまう）の原因になったため、駒落ち等の
            // 意図的な非標準開局のニーズは無いと確認のうえ削除。不一致がある間は対局を開始
            // できず、必ず「再撮影」か「手動キャリブレーション」で解消する構成にした。
            text = if (isMismatchChoice) {
                val target = if (isResume) "中断時点の盤面" else "初期配置"
                "認識結果が${target}と${mismatchCount}マス異なります（赤色塗りつぶし部）。\n" +
                    "駒を物理的に直した場合は「再撮影」を、盤と枠線がずれている等が原因と思われる場合は" +
                    "「手動キャリブレーション」を選んでください"
            } else {
        ...
        Row(
            ...
            if (isMismatchChoice) {
                // 7回目UAT課題②再テスト直後の訂正：「駒修正済、対局へ」（不一致の認識結果を
                // そのまま確定するボタン）は、駒落ち等の意図的な非標準開局のニーズが無いと
                // 確認できたため削除。不一致がある間は対局を開始できず、必ず「再撮影」か
                // 「手動キャリブレーション」で不一致が解消されるまでやり直す構成にした
                // （課題⑥・その再現で繰り返し発生した「撮り直さず確定してしまう」誤操作を
                // 文言の警告ではなく構造的に防ぐ）。
