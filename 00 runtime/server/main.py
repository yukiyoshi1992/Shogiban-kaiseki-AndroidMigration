...
# 入力を処理できる状態になるまでに数秒かかる、という挙動に対し、待機オーバーレイ
# （読み込み直後の数秒間、タップ対象を物理的に覆って強制的に待たせる）を試した。
# 5秒版は実際に症状を解決したが、毎回のページ遷移で5秒待たされる体感が悪く、
# 「タブ内最初の1回だけ待つ」という短縮策も実機検証で誤りと判明した
# （フルページロードのため毎回warmupコストが発生する）。
#
# **2026-06-23、最終判断**：ユーザーから「待たされるくらいなら2回タップの方が
# まし、元に戻そう」との明確な意思表示。待機オーバーレイ方式を撤回し、JSによる
# 介入を一切行わない、素の`<a href>`のみの状態に戻す。「2回タップが必要なことが
# ある」という症状自体は、この端末特有の制約として**仕様として受け入れる**。
_WEB_NAV_SCRIPT = ""
